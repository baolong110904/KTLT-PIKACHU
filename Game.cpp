...
void exitscreen()
{
    ...
    auto print1 = [&]()
    {
        ...
        while (i < 2)
        {
            ...
            gotoxy(left[choice * 3], top);
            putchar(word[i * 2]);
            gotoxy(left[choice * 3 + 1], top);
            cout << str[choice];
            gotoxy(left[choice * 3 + 2], top);
            putchar(word[i * 2 + 1]);
            if (!i++)
                choice = !choice;
        }
...
void instructionscreen()
{
    ...
    putchar(249);
    cout << " I Matching: +3 points";

    gotoxy(left1 + 40, top + 14);
    putchar(249);
    cout << " L Matching: +3 points";
    setcolor(Yellow);
    ...
    putchar(249);
    cout << " U Matching: +3 points";

    setcolor(Red);
    gotoxy(left1 + 10, top + 16);
    putchar(249);
    cout << " Not Matched: -3 points";

    setcolor(Red);
    gotoxy(left1 + 40, top + 16);
    putchar(249);
    cout << " Move suggestion: -2 points";
    ...
    while (true)
    {
        if (_kbhit())
        {
            char c = _getch();
            if (c == 'B' || c == 'b' || c == '\r')
            { // check if the user pressed "B" or "b"
                system("cls");
                setConsolecolor(LIGHT_YELLOW, BLACK);
                break;
            }
...
bool linecheck(pokemon **map, int x1, int y1, int x2, int y2)
{
    if (x1 == x2)
    {
        ...
        if ((dem == 1) && ((!map[x1][y1].isValid && map[x2][y2].isValid) || (map[x1][y1].isValid && !map[x2][y2].isValid)))
        { // sai
            return true;
        }
        else if (dem == 0)
        ...
        for (int j = 1; j < widthEz - 1; j++)
        {
            ...
            for (int k = 1; k < heightEz - 1; k++)
            {
                for (int l = 1; l < widthEz - 1; l++)
                {
                    ...
                    if (map[i][j].chr == map[k][l].chr)
                    {
                        if (allCheckHard(map, i, j, k, l) == true)
                        {
                            guidePos[0].x = j;
                            guidePos[0].y = i;
                            guidePos[1].x = l;
...
void Game::moveEasy(pokemon **map, position &pos, int &status, player &p, position selectedPos[], int &couple, int heightEz, int widthEz)
{
    ...
    {                        // neu ko phai la dau mui ten
        ...
        else if (temp == ENTER_KEY || temp == SPACE_KEY)
        { // neu la Enter
        ...
        { // neu la Enter
            ...
            else
            {
                ...
                { // neu da chon 1 cap
                    if (map[selectedPos[0].y][selectedPos[0].x].chr == map[selectedPos[1].y][selectedPos[1].x].chr)
                    { // neu cap nay hop nhau
                        if (allCheckHard(map, selectedPos[0].y, selectedPos[0].x, selectedPos[1].y, selectedPos[1].x))
                        {
                            PlaySound("music\\ye.wav", NULL, SND_ASYNC);
                            p.point += 3;
                            gotoxy(123, 0);
...
void Game::generateMapHard(pokemon **&map)
{
    ...
    int totalCell = ((mapHardWidth - 2) * (mapHardHeight - 2)) / 2;
...
void Game::printCongratulationBoardEasy(player &p)
{
    ...
    auto print1 = [&]()
    {
        ...
        while (i < 2)
        {
            ...
            gotoxy(left[choice * 3], top + 6);
            putchar(word[i * 2]);
            gotoxy(left[choice * 3 + 1], top + 6);
            cout << str[choice];
            gotoxy(left[choice * 3 + 2], top + 6);
            putchar(word[i * 2 + 1]);
            if (!i++)
                choice = !choice;
        }
...
void Game::printLoseEasy(player &p)
{
    ...
    auto print1 = [&]()
    {
        ...
        {
            PlaySound("music\\click.wav", NULL, SND_ASYNC | SND_NOSTOP);
            gotoxy(left[choice * 3], top + 1);
            putchar(word[i * 2]);
            gotoxy(left[choice * 3 + 1], top + 1);
            cout << str[choice];
            gotoxy(left[choice * 3 + 2], top + 1);
            putchar(word[i * 2 + 1]);
            if (!i++)
                choice = !choice;
        }
